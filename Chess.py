...
#This function prints the initial state of the board
...
#Printing instructions on how to interact with the board to play Chess
...
main_board = copy.copy(STARTING_PIECES) #Receives a copy of the STARTING_PIECES list
while True:  #Creates an infinite loop to allow a user to enter inputs
    print_chessboard(main_board)
    response = input('> ').split()


    #Correspondent action to the "move" command
    if response[0] == 'move':
        main_board[response[2]] = main_board[response[1]]
        del main_board[response[1]]
    
    #Correspondent action to the "remove" command
    elif response[0] == 'remove':
        del main_board[response[1]]
    
    #Correspondent action to the "set" command
    elif response[0] == 'set':
        main_board[response[1]] = response[2]

    #Correspondent action to the "reset" command
    elif response[0] == 'reset':
        main_board = copy.copy(STARTING_PIECES)

    #Correspondent action to the "clear" command
    elif response[0] == 'clear':
        main_board = {}
    
    #Correspondent action to the "fill" command
    elif response[0] == 'fill':
        for y in '87654321':
            for x in 'abcdefgh':
                main_board[x + y] = response[1]

    #Correspondent action to the "quit" command
